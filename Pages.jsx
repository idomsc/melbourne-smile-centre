    ...
    <section data-nav-theme="dark" className="msc-team-section" style={{
      ...
      <div className="msc-inner-pad" style={{ maxWidth:1280, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"flex-end",
          ...
          <style>{`@keyframes dragPulse{0%,100%{transform:translateX(0)}50%{transform:translateX(5px)}}`}</style>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
            style={{ animation:"dragPulse 1.6s ease-in-out infinite", flexShrink:0 }}>
            <path d="M5 2v5M7.5 3v4M10 4v3M2.5 5v3.5C2.5 10.5 4 12 7 12s4.5-1.5 4.5-3.5V7"
              stroke="rgba(217,185,135,0.55)" strokeWidth="1.2" strokeLinecap="round"/>
          </svg>
          <span style={{ fontSize:9, color:"rgba(217,185,135,0.45)", fontFamily:"var(--msc-font-text)",
            fontWeight:700, letterSpacing:"0.14em", textTransform:"uppercase" }}>scroll to explore</span>
          <svg width="18" height="8" viewBox="0 0 18 8" fill="none">
            <path d="M1 4h16M12 1l4 3-4 3" stroke="rgba(217,185,135,0.35)" strokeWidth="1.2"
              strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
      </div>

      {/* Single horizontally scrollable track — bleeds past right edge */}
      <TeamScrollTrack onNavigate={onNavigate} />
    </section>
  );
}

function TeamScrollTrack({ onNavigate }) {
  ...
  /* Card sizes: dentists fill ~1/3 viewport each; hygienists smaller */
  const dentistW = "calc((100vw - 128px) / 3)";
  const hygW = "calc((100vw - 128px) / 4.2)";
    ...
    <div style={{ position:"relative" }}>
      ...
      <div
        ...
        style={{
          ...
          paddingLeft:32, paddingRight:32, paddingBottom:48,
          scrollbarWidth:"none", msOverflowStyle:"none",
        }}
      >
        {/* Dentist cards */}
        {DENTISTS_DATA.map((d, i) => (
          <div key={d.name} style={{ flexShrink:0, width:dentistW }}>
        ...
        <div style={{
          ...
          width:1, background:"rgba(255,254,251,0.08)",
          margin:"0 6px",
        }}/>

        {/* Hygienist label + cards */}
        <div style={{ flexShrink:0, display:"flex", flexDirection:"column", gap:10, justifyContent:"flex-end" }}>
          <div style={{ fontSize:9, color:"rgba(245,237,224,0.30)", letterSpacing:"0.18em",
            textTransform:"uppercase", fontWeight:700, fontFamily:"var(--msc-font-text)",
            paddingBottom:4, marginBottom:2 }}>Hygienists</div>
          <div style={{ display:"flex", gap:14 }}>
            {HYGIENISTS_DATA.map((h, i) => (
              <div key={h.name} style={{
                flexShrink:0, width:hygW, borderRadius:16, overflow:"hidden",
                background:"rgba(20,18,15,0.85)", border:"1px solid rgba(255,254,251,0.07)",
                aspectRatio:"3/4", position:"relative",
                opacity:0, animation:`aofFadeUp 400ms ${300 + i*80}ms ease both`,
              }}>
                {h.photo && (
                  <img src={h.photo} alt={h.name} style={{
                    position:"absolute", inset:0, width:"100%", height:"100%",
                    objectFit:"cover", objectPosition:"top center", mixBlendMode:"lighten",
                  }} onError={(e) => { e.target.style.display="none"; }} />
                )}
                <div style={{
                  position:"absolute", inset:0,
                  background:"linear-gradient(0deg, rgba(14,12,9,0.88) 0%, rgba(14,12,9,0.10) 60%, transparent 100%)",
                }}/>
                <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:"14px" }}>
                  <div style={{ fontSize:9, color:"var(--msc-primary-on-dark)", letterSpacing:"0.12em",
                    textTransform:"uppercase", fontWeight:700, fontFamily:"var(--msc-font-text)",
                    marginBottom:3, opacity:0.65 }}>Hygienist</div>
                  <div style={{ fontFamily:"var(--msc-font-display)", fontSize:14,
            ...
            <div onClick={() => onNavigate && onNavigate("team")} style={{
              flexShrink:0, width:"120px", borderRadius:16, aspectRatio:"3/4",
              background:"rgba(176,135,84,0.07)", border:"1px solid rgba(176,135,84,0.18)",
              display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
              gap:10, cursor:"pointer", padding:"16px",
              transition:"background 200ms ease, border-color 200ms ease",
            }}
            onMouseEnter={e=>{ e.currentTarget.style.background="rgba(176,135,84,0.13)"; e.currentTarget.style.borderColor="rgba(176,135,84,0.35)"; }}
            onMouseLeave={e=>{ e.currentTarget.style.background="rgba(176,135,84,0.07)"; e.currentTarget.style.borderColor="rgba(176,135,84,0.18)"; }}
            >
              <div style={{ width:36, height:36, borderRadius:"50%",
                border:"1px solid rgba(176,135,84,0.40)",
                display:"flex", alignItems:"center", justifyContent:"center" }}>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M2 7h10M8 3l4 4-4 4" stroke="var(--msc-primary-on-dark)" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              <span style={{ fontSize:9, color:"rgba(217,185,135,0.60)", textAlign:"center",
                fontFamily:"var(--msc-font-text)", fontWeight:700, letterSpacing:"0.10em",
                textTransform:"uppercase", lineHeight:1.6 }}>Full team</span>
            </div>
          </div>
        </div>
      ...
      {/* Right-edge fade */}
      <div style={{
        position:"absolute", right:0, top:0, bottom:0, width:100, pointerEvents:"none",
